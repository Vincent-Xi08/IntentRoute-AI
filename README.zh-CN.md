...
> **项目状态：v0.31.0 预览版。** IntentRoute AI 适合测试与早期采用，尚未经过大规模生产验证。AI 输出可能不完整或有误；每条生成的规则都经过本地校验、以禁用状态写入，必须由你显式启用。

v0.26.0 通过 DWM 沉浸式暗色属性为全部对话框加上深色系统标题栏，并首次对条件编辑器做端到端实测（v0.13 引入以来第一次真实打开）。v0.25.0 将应用内全部 20 处弹窗替换为主题化深色对话框并修复 v0.17 进场动画崩溃；v0.24.0 以 100% + 125% 双屏实测声称混合 DPI 验证；v0.15–v0.23 依次交付导入预览、共享深色设计系统、组件打磨、主题色页头、批量操作、键盘交互与审查加固。每个发布包仍内嵌 v0.9.0 引入的未签名 `provenance.json` 构建溯源清单；代码签名本身仍需要证书。
...
1. 从 [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases) 下载 `IntentRoute-AI-v0.31.0-win-x64.zip` 与 `.sha256` 文件。
2. 校验校验和。
...
`rust/` 工作区是分阶段 Rust 重构：`intentroute-core` 移植确定性核心（与 C# 存储格式 JSON 兼容的规则模型、约束校验、规则身份、导入规划、规范运行顺序，以及带回环上游校验与密码脱敏的 sing-box 配置构建器），测试向量从 C# 测试套件移植；`intentroute-cli` 提供本地 `check` / `order` / `policy` / `import-preview` / `build-config` 子命令；`intentroute-gui`（阶段 3a+3b）是只读规则控制台——自动加载产品配置，提供规范顺序、过滤、排序、校验标记与详情面板；界面为中文，运行时加载系统 CJK 字体（无字体时回落英文，可用 `INTENTROUTE_GUI_LANG` 覆盖）。`./scripts/test-rust.ps1` 在 CI 中运行 `cargo test` 与发布构建；钉版真实 sing-box 门禁会同时校验 C# 与 Rust 两份构建产物；在 GUI 达到同等功能之前，WPF 应用仍是受支持的产品。
...
- 界面以中文为一等语言：除策略发现标题与持久化默认代理名（作为稳定分析标识与配置数据保持中文）外，全部用户可见字符串跟随语言偏好（553 资源键）。
- 键盘导航带可见焦点态、辅助技术名称与冒烟级 Tab/方向键覆盖，并断言 Per-Monitor V2 DPI 感知；混合 DPI 视觉布局验证已于 2026-09-07 执行并记录（100% + 125% 双屏，见 docs/mixed-dpi-verification.md）。完整屏幕阅读器行为尚未宣称。
