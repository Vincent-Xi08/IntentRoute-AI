...
> **项目状态：v0.20.0 预览版。** IntentRoute AI 适合测试与早期采用，尚未经过大规模生产验证。AI 输出可能不完整或有误；每条生成的规则都经过本地校验、以禁用状态写入，必须由你显式启用。
...
1. 从 [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases) 下载 `IntentRoute-AI-v0.20.0-win-x64.zip` 与 `.sha256` 文件。
2. 校验校验和。
