...
`rust/` 工作区是分阶段 Rust 重构：`intentroute-core` 移植确定性核心（与 C# 存储格式 JSON 兼容的规则模型、约束校验、规则身份、导入规划、规范运行顺序，以及带回环上游校验与密码脱敏的 sing-box 配置构建器），测试向量从 C# 测试套件移植；`intentroute-cli` 提供本地 `check` / `order` / `policy` / `import-preview` / `build-config` 子命令；`intentroute-gui` 是与 WPF 共用核心的规则控制台：规范顺序、过滤、排序、校验标记、详情面板，全部有界编辑路径（启停/模式/约束/增删/上下移动/代理服务器/全局模式，均经管理锁事务），策略体检（身份重复、遮蔽、部分重叠、范围过宽）、路由推演、进程列表，以及运行日志监控——监控页只执行显式选定的 sing-box v1.13+ 可执行文件（探测版本、`check` 通过后才托管 `run`），托管期间持有与 WPF 相同的运行锁（双向互斥），自身编辑直接在持锁下进行并把进程标记为「配置已过期」，可一键应用配置并重启（替换失败自动回滚旧配置与旧进程，孤儿进程凭 PID+启动时间+可执行文件三重证明恢复清除），控制台输出按行脱敏进有界环形缓冲，停止/失败即删除含密码的生成配置；界面为中文，运行时加载系统 CJK 字体（无字体时回落英文，可用 `INTENTROUTE_GUI_LANG` 覆盖）。`./scripts/test-rust.ps1` 在 CI 中运行 `cargo test` 与发布构建；钉版真实 sing-box 门禁会同时校验 C# 与 Rust 两份构建产物；在 GUI 达到同等功能之前，WPF 应用仍是受支持的产品。
