...
> **Project status: v0.25.0 preview.** IntentRoute AI is useful for testing and early adoption, but it has not yet demonstrated broad production usage. AI output can be incomplete or wrong. Every generated rule is locally validated, added disabled, and must be explicitly enabled by the user.

v0.25.0 replaces all twenty in-app message boxes with themed dark dialogs and fixes a latent v0.17 crash in the shared dialog entrance animation (Window.RenderTransform is forbidden in WPF). v0.24.0 claimed mixed-DPI visual validation with a recorded 100% + 125% dual-display run; v0.19–v0.23 delivered batch mode buttons, log severity coloring, keyboard interaction, and review hardening; v0.15–v0.18 added the import preview, shared dark design system, component polish, and theme-colored page identity. Every release archive still embeds the unsigned `provenance.json` build inventory introduced in v0.9.0; code signing itself still requires a certificate.
...
1. Download `IntentRoute-AI-v0.25.0-win-x64.zip` and its `.sha256` file from [Releases](https://github.com/Vincent-Xi08/IntentRoute-AI/releases).
