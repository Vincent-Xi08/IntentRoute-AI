...
The `rust/` workspace is a phased Rust migration: `intentroute-core` ports the deterministic core (rule model JSON-compatible with the C# store, constraint validation, rule identity, import planning, canonical runtime order, and the sing-box configuration builder with loopback-only upstream validation and password redaction) with test vectors ported from the C# suites; `intentroute-cli` offers local `check` / `order` / `policy` / `import-preview` / `build-config` subcommands; and `intentroute-gui` is the rules console sharing that core: canonical order, filtering, sorting, validation flags, and a detail panel; every bounded edit path (enable/mode, constraints, add/delete, move up/down, proxy servers, global mode — each through a management-locked transaction); policy check (identity duplicates, shadowing, partial overlap, broad scope); route simulation; process list; and the runtime log monitor — the monitor executes only an explicitly selected sing-box v1.13+ executable (version probe, then `check`, before hosting `run`), holds the same runtime lock the WPF runtime uses (mutual exclusion both ways; its own edits run directly under that ownership and mark the process stale, with one-click apply-and-restart that rolls back to the previous config and process when the replacement fails, and orphan recovery proven by PID + start time + executable identity), redacts captured console lines into a bounded ring buffer, and deletes the password-bearing generated config on stop/failure, with a Chinese UI backed by a runtime-loaded CJK system font (English fallback, `INTENTROUTE_GUI_LANG` override). `./scripts/test-rust.ps1` runs `cargo test` and a release build in CI, and the pinned real-sing-box gate validates the Rust builder output alongside the C# output; the WPF application remains the supported product until GUI parity is reached.
