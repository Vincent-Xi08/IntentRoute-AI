...
- Rust migration, parity slice 13: shell-replacement preparation. The Rust console gains a CI smoke gate mirroring the WPF packaged smoke test: `scripts/smoke-test-rust-gui.ps1` launches the release binary, proves the render loop actually ran (the app writes a marker file after three rendered frames when `INTENTROUTE_GUI_SMOKE_MARKER` names a path — inert otherwise), and requires a clean zero exit. The gate runs in two modes: windowed (default — asserts the main window, its title, and that a normal close request is accepted; verified locally end-to-end including the real configuration autoload) and `-Headless` (used in CI, because hosted runners expose no GPU adapter for any backend — the update loop was factored into `render_ui` and the app can drive the same startup, font, and layout passes off-screen via `egui::Context::run`, then exit zero; startup failures carry the app's own stderr through a cmd-wrapped diagnostic rerun). A renderer override (`INTENTROUTE_GUI_RENDERER=wgpu`, eframe wgpu feature enabled) also supports GPU-less real machines. The stale "(read-only)" suffix was dropped from both window titles. CI runs the headless gate after the Rust tests and uploads the release console as an `IntentRoute-AI-rust-console` preview artifact alongside the WPF package. `docs/RUST_PARITY.md` records the feature-parity matrix (all rule editing, policy intelligence, simulation, process list, and the managed runtime are at semantic parity), the deliberate divergences (per-session executable approval, start-time-pinned apply, lazy progress states), and the concrete checklist for promoting the console to the supported front end (import/batch/AI panels, panic-to-redacted-diagnostic hook, release pipeline, signing).

- Rust migration, parity slice 12: the final runtime semantics — apply-with-rollback, orphan recovery, and the RunningStale ladder. The managed child's PID, .NET-epoch start ticks, and executable path are recorded in `sing-box.runtime-state.json` with the same field names the WPF runtime writes, so either manager can recover the other's orphan: on start, recovery runs under the freshly acquired lock, proves identity (PID + start time within ±1 s via `GetProcessTimes`, plus executable path when both sides know it), terminates the process, and removes the state file. `ManagedRuntime::apply(config)` ports `ApplyAsync` pinned to the start-time executable: write candidate → `check` → atomic promote → kill old + start replacement (new kill-on-close job, fresh state file) → settle; a replacement that exits during startup restores the previous config bytes and restarts the previous executable (`RunningStale` with the "restored and restarted" error), and only a failed rollback deletes the config and converges to `Failed`; a check failure leaves the running process and old config untouched (stale, not failed). `mark_running_stale` records a redacted stale reason, and a successful apply clears it. The console now follows the WPF single-owner model in both directions: while it manages a process its own edits run directly under its lock ownership (no more "stop first" refusal) and mark the process stale, and the monitor panel gains 应用配置并重启 (apply config & restart) plus a yellow stale state. End-to-end verified against the real configuration with a config-sensitive fake: apply replaced the process, a stale mark was set and cleared, a crash-on-startup config rolled back to the previous bytes and process, a check failure kept the old PID, and stop removed config + state file. Core suite: 131 tests (+9). Runtime semantics parity is complete; the residual gap is product polish (executable switching during apply, transient progress states).

- Rust migration, parity slice 11: runtime log monitor — the deepest WPF-side capability — in two core modules plus a console panel. `intentroute_core::runtime_log` (13 tests) ports the WPF `RuntimeLogFilter` exactly: logrus level tokens (with optional timestamp prefix, `Information`-style overlong tokens rejected, unknown lines treated as Info), level+case-insensitive-search filtering, and export rendering that re-redacts each message. `intentroute_core::singbox` (19 tests) ports the execution-facing half of `SingBoxRuntime`/`SingBoxExecutionBackend`: discovery only *lists* candidates (env → legacy env → beside the app → PATH) and never executes them; the version probe requires a recognizable ≥ 1.13.0 banner; `check -c` runs on a candidate file before promotion; and `ManagedRuntime::start` acquires the same `sing-box.runtime.lock` the WPF runtime holds, atomically promotes the full (password-bearing, never-displayed) generated config, spawns `sing-box run -c` with `CREATE_NO_WINDOW` inside a kill-on-close Job Object (whole-tree termination without helper processes), redacts every captured stdout/stderr line at capture into a bounded ring (floor 32, default 200 — the WPF `Math.Max(32,…)` semantics), lazily converges to Failed with an exit-code error and deletes the generated config when the child dies on its own, and stops by killing the tree, deleting the config, and (on drop) releasing the lock. Errors are secret-redacted and truncated to 800 chars, exactly like the WPF helpers. The console's **运行日志** panel adds: explicit executable selection (browse/probe — no unapproved path is ever executed), start/stop, live status (state/PID/version/error), level filter, search, auto-scroll, clear, and export (re-redacted); the start path builds from a freshly workspace-loaded snapshot so DPAPI passwords are decrypted before the runtime file. End-to-end verified against the real configuration and a fake sing-box: decrypt → build → probe → check → run → capture (redacted) → filter → stop → config deleted, lock released. Core suite: 122 tests (+32).

- Rust migration, parity slice 10: partial-overlap detection in the policy check, plus two containment-direction corrections in the shadowing proof. A rule pair **provably intersects without either covering the other** when process, protocol, and ports overlap and the destination constraints intersect via tree containment (a domain token contains or is contained by the other side's token; same for IP CIDRs) — domain-versus-IP constraint pairs can never be proven to intersect. The finding reports that shared traffic hits the earlier rule, is severity Info for equal modes and Warning for differing modes, and is explicitly labeled a non-proof hint: traffic outside the intersection still follows each rule's own scope. Reverse-containment pairs (a restricted earlier rule followed by an unrestricted later one) stay silent, matching the WPF engine. While porting, two slice-9 false-positive shadow reports were fixed to match the WPF `Contains` semantics: a port-restricted or destination-restricted earlier rule no longer "shadows" an unrestricted later rule. The CLI gains `intentroute policy <rules.json>` printing all findings headlessly. Core suite: 90 tests (+12). Live-verified via the CLI on a synthetic four-rule file: broad-scope (Info), shadowing (Critical), and cross-mode overlap (Warning) all fired with correct severities and positions.

- Rust migration, parity slice 9: shadowing and broad-scope detection in the policy check. A core `policy_findings` module (10 tests) proves **exact-superset shadowing** — an earlier rule in canonical order covers everything a later rule matches (process is a superset, domain `*.suffix` ⊇ subdomain, IP CIDR ⊇ narrower CIDR or member IP, port range ⊇ contained range, protocol Both ⊇ TCP/UDP) — and **broad scope** (an enabled rule with no destination constraints covers all traffic for its process). The policy panel displays both findings with the rule positions and the covering rule's name. Live-verified: the real configuration's only rule correctly triggers the broad-scope finding. Core suite: 78 tests.

- Rust migration, parity slice 8: process list in the console. A core `process` module snapshots running processes via Toolhelp32 (explicit W bindings matching the v0.11 WPF fix — 3 tests verify current-process inclusion, no mojibake from the ANSI/Unicode mismatch, and PID uniqueness) with per-process executable paths via `QueryFullProcessImageNameW`. The console's **进程列表** panel shows the snapshot with a live search filter (name/PID), a reload button, and a per-process **添加为规则** button that creates an enabled Proxy-mode rule through the same management-locked transaction with full-identity duplicate checking. Live-verified: the panel populated with real processes and paths. Core suite: 68 tests.

- Rust migration, parity slice 7: global mode toggle in the console toolbar. The current mode (默认直连 / 默认代理) displays inline; the ⇄ button offers the other mode behind a confirmation dialog stating the traffic-impact consequence, then commits through the same management-locked transaction. The toolbar label refreshes from the published snapshot. Live-verified: toggled to ProxyAll (disk `GlobalMode: 0`), label updated to 默认代理, then toggled back to DirectAll (disk `GlobalMode: 1`) and the original state restored.

- Rust migration, parity slice 6: route decision simulator in the console. A new core `route_sim` module (10 tests) evaluates one exact process / domain-or-IP / port / TCP-UDP query against **enabled** rules in Canonical Runtime Order and returns the first proven match or the global fallback — the same three-valued semantics as the WPF simulator (match / no-match / invalid-query), strictly local with no DNS resolution, probes, or traffic observation. The console's **路由推演** panel exposes this with inline inputs (process, destination + domain/IP toggle, port, TCP/UDP) and a one-click result. Live-verified: the real configuration's only rule correctly matched with "代理" for its own process name, and an empty destination was rejected with a localized invalid-query message. Core suite: 65 tests.

- Rust migration, parity slice 5: policy check panel in the console. **策略体检** toggles a panel with KPI stat cards (active / critical / warning / disabled) and local deterministic findings — exact identity duplicates (using the shared `rule_identity_key`), proxy rules without an enabled server, global-proxy mode without a server, and unsupported proxy-chain references. All analysis uses existing core modules and is entirely local; the full WPF engine's shadowing, containment, and broad-scope findings remain WPF-only until ported. Live-verified against the real configuration (1 active rule, 0 findings, clean report shown).

- Rust migration, parity slice 4: rule priority reordering. The detail panel gains **上移 / 下移** (move up / move down) buttons that implement the exact WPF `MoveRule` semantics: swap the rule with its neighbor in Canonical Runtime Order (all rules, not just enabled), rewrite the persisted list in that order, and reassign every priority as `(i+1)*10`. Moves execute immediately through the management-locked transaction (no dialog — the click itself is the explicit intent, matching the WPF context-menu behavior); out-of-range moves are silent no-ops. Live-verified against a two-rule configuration: the lower rule moved to the top with both priorities correctly renumbered on disk, then restored.

- Rust migration, parity slice 3: proxy-server editing. **代理服务器** opens a dialog listing every server with its editable fields — type (SOCKS5/HTTP/HTTPS), loopback host, port, username, password (masked), enabled — with live loopback/port validation using the exact core `try_normalize` the builder enforces; Save stays disabled until every server parses. Each changed server commits through its own management-locked transaction; the engine re-validates loopback/port and the serializer re-encrypts the password into a fresh DPAPI envelope on every save. Live-verified against the real configuration: port edited to 10809 committed to disk, host untouched, restored to 10808.

- Rust migration, parity slice 2: rule add and delete in the console. **添加规则** opens a dialog (exact process name or `*`, mode selector, Save disabled until the name passes the WPF-parity shape check) that creates an enabled rule with a fresh id and next-tens priority; the full-identity duplicate check runs against the freshly loaded on-disk state inside the management-locked transaction, so a same-identity rule added by another editor after this view loaded is still refused. **删除规则** (red, in the detail panel) shows an explicit irreversible-action confirmation before the same locked transaction. Live-verified against the real configuration: add committed to disk with correct priority, delete confirmed and removed, and the original single-rule state restored.

- Rust migration, parity slice: the rules console gains a constraints editor matching the WPF RuleEditWindow. **编辑约束** opens a dialog for hosts / IP-CIDR / ports / protocol (any/TCP/UDP/Both) with live validation through the shared core validator — the error text is localized and the Save button stays disabled until every field parses, exactly like the WPF gate — and Save routes through the same management-locked load→commit transaction as the other edits (the validated dialog itself is the confirmation). Verified end-to-end against the real configuration: constraints committed to disk, the row summary/detail panel refreshed from the published snapshot, and a second edit restored the original empty constraints. Core suite unchanged at 55 tests (validation semantics were already covered by the ported vectors).

- Rust migration, phase 3c GUI slice: the rules console gains bounded editing. The detail panel can toggle a rule's enabled state and set its mode (proxy/direct/block); every edit shows a confirmation dialog stating the exact intention, then runs one management-locked load→commit transaction through the tested workspace engine (`runtime_lock` ports the `sing-box.runtime.lock` exclusive-handle semantics from the WPF runtime). Verified end-to-end against the real configuration: while a WPF instance holds the lock the edit is blocked with a localized message, and after the WPF instance exits the same edit commits atomically and the view refreshes from the published snapshot. 2 new lock tests bring the core suite to 55. No other fields are editable from this shell yet.

- Rust migration, phase 3c (engine slice): the transactional persistence boundary is ported to `intentroute-core`. `dpapi` reproduces the C# password envelope (`"dpapi:" + base64`, CryptProtectData/CurrentUser/no-entropy FFI, legacy plaintext one-time passthrough, fail-closed on malformed or foreign-user ciphertext); `workspace` reproduces `LoadPreservingInvalidFile` (Missing / Loaded / Unusable with `corrupt-*.bak` recovery copy and DPAPI decrypt at the load boundary) and `commit` (clone → mutate → normalize → the same invariants incl. the full builder dry-run → DPAPI-protected serialize → same-directory temp + `ReplaceFileW` atomic replace → publish, leaving memory and disk untouched on any failure). 16 new tests (atomicity against a locked destination file, duplicate-id/chain/loopback rejection with byte-identical disk, encrypted-at-rest canaries that are random per run). GUI writes are deliberately not wired yet — coordination with a running WPF instance is the next slice.

- Rust migration, phase 3b: the rules console now renders a full Chinese UI. A CJK system font (Microsoft YaHei → SimHei → SimSun, in that order) is loaded at runtime — nothing is bundled into the repository — and the UI language follows font availability, falling back to English rather than placeholder boxes when no CJK font exists; `INTENTROUTE_GUI_LANG=zh|en` overrides the choice. All shell strings (menu, columns, banner, detail panel, status line, constraint-error names) are localized in-crate, with the window title bilingual. Verified live against the real product config, including the Chinese detail panel.
