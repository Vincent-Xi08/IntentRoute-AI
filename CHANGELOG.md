...
- Rust migration, parity slice 13: shell-replacement preparation. The Rust console gains a CI smoke gate mirroring the WPF packaged smoke test at the depth egui supports: `scripts/smoke-test-rust-gui.ps1` launches the release binary, asserts the main window appears with the expected title, proves the render loop actually ran (the app writes a marker file after three rendered frames when `INTENTROUTE_GUI_SMOKE_MARKER` names a path — inert otherwise), then requires a normal window-close request to be accepted with a clean zero exit. The stale "(read-only)" suffix was dropped from both window titles — the console has been a full editor since the bounded-edit slices. CI now runs the smoke gate after the Rust tests and uploads the release console as an `IntentRoute-AI-rust-console` preview artifact alongside the WPF package. `docs/RUST_PARITY.md` records the feature-parity matrix (all rule editing, policy intelligence, simulation, process list, and the managed runtime are at semantic parity), the deliberate divergences (per-session executable approval, start-time-pinned apply, lazy progress states), and the concrete checklist for promoting the console to the supported front end (import/batch/AI panels, panic-to-redacted-diagnostic hook, release pipeline, signing). Verified locally end-to-end: window + title + three rendered frames (with the real configuration autoloaded) + clean close + zero exit, no orphaned process.
